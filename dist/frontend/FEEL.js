import { _DMiNer_, DMiNer_error, DMN_type_reference_, Is_DMN_type_reference_, Is_DMN_DecisionTable, Is_DMN_LiteralExpression, Is_DMN_UnaryTests, Name_of_DMN_Decision, Name_of_DMN_InputClause, Name_of_DMN_OutputClause, Type_of_DMN_OutputClause, Hit_policy, Trace } from "../common/Settings.js";
import IMICROS_FEEL_interpreter from "./IMICROS_FEEL_interpreter.js";
// window.alert(FEELin.unaryTest("< 5", {"?": 0}));
class FEEL {
    static Evaluate(decision, data) {
        if (Is_DMN_DecisionTable(decision.decisionLogic)) {
            const decision_table = decision.decisionLogic;
            data.forEach(datum => {
                FEEL._Rejected_rules.clear();
                const keys = Object.keys(datum);
                decision_table.input.every((input_clause, input_clause_index) => {
                    ...
                    if (index !== -1) {
                        ...
                        if (input_clause.hasOwnProperty('inputExpression') && Is_DMN_LiteralExpression(input_clause.inputExpression)) {
                            const literal_expression = input_clause.inputExpression;
                            if (literal_expression.text === Object.keys(datum)[index] && Is_DMN_type_reference_(literal_expression.typeRef))
                                evaluation &&= literal_expression.typeRef === typeof value;
                        }
                ...
                decision_table.rule.filter(rule => !FEEL._Rejected_rules.has(rule)).every((rule) => {
                    ...
                    decision_table.output.forEach((output_clause) => {
                        ...
                        const value = type === DMN_type_reference_.INTEGER || type === DMN_type_reference_.LONG
             ...
             */
            const literal_expression = decision.decisionLogic;
...
FEEL._FEEL_EXPRESSION = "FEEL_EXPRESSION";
FEEL._HYPHEN = "-";
FEEL._Rejected_rules = new Set;
export default FEEL;
